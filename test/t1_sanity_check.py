...
from models.dtmodel.layers import (
    DTAttention,
    DTFlashAttention2,
    RMSNorm,
    SwiGLUMLP,
...
class SanityChecker:
    ...
    def _create_test_config(self) -> DTConfig:
        """테스트용 config 생성"""
        config = DTConfig(
            vocab_size=self.config.vocab_size,
            hidden_size=self.config.hidden_size,
    ...
    def test_layer_structure(self) -> bool:
        ...
        model = DTForCausalLM(config).to(self.device)
    ...
    def test_rotary_embedding(self) -> bool:
        ...
        config = self._create_test_config()
        rope = DTRotaryEmbedding(config).to(self.device)
    ...
    def test_attention_forward(self) -> bool:
        ...
        config = self._create_test_config()
        # attn = DTAttention(config, layer_idx=0).to(self.device)
        attn = DTFlashAttention2(config, layer_idx=0).to(self.device)
